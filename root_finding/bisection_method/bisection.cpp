...
int main() {
   ...
   float ntol = -tol;            // accuracy tolerance (negative)
   ...
   for (i = 0; i < N; i++) {
      ...
      if (ntol < fc && fc < tol) break;   // Solution found within specified tolerance
      else if (f(a)*fc < 0) b = c;        // Update [a,b] --> [a,c]
      else a = c;                         // Update [a,b] --> [c,b]
   };

   // Output results
   if (i == N) {                          // solution not found
