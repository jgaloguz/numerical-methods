...
// Convolution function 2D arrays
...
// Pad image (currently not being used)
/*
...
void non_max_supp(double **Ln, double **Ld, double **L, int h, int w, int margin_h, int margin_w)
{
   ...
   for (i = 0; i < h; i++) {
      for (j = 0; j < w; j++) {
         ...
         // Compare
         if (Ln[k][l] >= pgrad && Ln[k][l] >= ngrad) L[k][l] = Ln[k][l];           // Keep maximum values only
      };
   };
};

// Find maximum of 2D array
double max2D(double **A, int h, int w, int margin_h, int margin_w)
{
   ...
   for (i = 0; i < h; i++) {
      for (j = 0; j < w; j++) {
         k = i+margin_h;
         l = j+margin_w;
         if (A[k][l] > max) max = A[k][l];      // Update max
      };
   };
   return max;
};

// Apply two-threshold filter
void two_threshold(double **L, double ht, double lt, int h, int w, int margin_h, int margin_w)
{
   ...
   for (i = 0; i < h; i++) {
      for (j = 0; j < w; j++) {
         ...
         l = j+margin_w;
         Lij = L[k][l]/max;
         if (Lij <= lt) L[k][l] = 0;            // Suppress if under low threshold
         else if (Lij < ht) L[k][l] = 0.5;      // Weak edge if between low and high threshold
         else L[k][l] = 1;                      // Strong edge if higher than
      };
   };
};
...
bool check2D(double **A, double **B, int h, int w, int margin_h, int margin_w)
{
   ...
   for (i = 0; i < h; i++) {
      for (j = 0; j < w; j++) {
         k = i+margin_h;
         l = j+margin_w;
         if (A[k][l] != B[k][l]) return true;      // TRUE if matrices are NOT equal
      };
   };
...
void hysteresis(double **L, double **edges, int h, int w, int margin_h, int margin_w)
{
   ...
   do {
      ...
      for (i = 0; i < h; i++) {
         for (j = 0; j < w; j++) {
            ...
            if (L[k][l] == 1) {
               if (L[k][l+1] == 0.5) L[k][l+1] = 1;
               if (L[k-1][l+1] == 0.5) L[k-1][l+1] = 1;
               if (L[k-1][l] == 0.5) L[k-1][l] = 1;
               if (L[k-1][l-1] == 0.5) L[k-1][l-1] = 1;
               if (L[k][l-1] == 0.5) L[k][l-1] = 1;
               if (L[k+1][l-1] == 0.5) L[k+1][l-1] = 1;
               if (L[k+1][l] == 0.5) L[k+1][l] = 1;
   ...
   for (i = 0; i < h; i++) {
      for (j = 0; j < w; j++) {
         k = i+margin_h;
         l = j+margin_w;
         if (edges[k][l] == 0.5)  edges[k][l] = 0;       // set isolated weak edges to zero
      };
   };
};
